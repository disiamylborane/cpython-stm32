...
//    const dfa *d;
//
//    g->g_dfa = (const dfa *)PyObject_REALLOC(g->g_dfa,
//                                        sizeof(const dfa) * (g->g_ndfas + 1));
//    if (g->g_dfa == NULL)
//        Py_FatalError("no mem to resize const dfa in adddfa");
//    d = &g->g_dfa[g->g_ndfas++];
//    d->d_type = type;
...
//void
//addarc(const dfa *d, int from, int to, int lbl)
//{
//    state *s;
//    arc *a;
//
