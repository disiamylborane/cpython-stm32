                  ...
                  int lineno, int col_offset, int *expected_ret)
{
    ...
    for (;;) {
        /* Fetch the current const dfa and state */
        const dfa *d = ps->p_stack.s_top->s_dfa;
        state *s = &d->d_state[ps->p_stack.s_top->s_state];

        D(printf(" const dfa '%s', state %d:",
...
#endif
#endif
            /* Pop this const dfa and try again */
            s_pop(&ps->p_stack);
            D(printf(" Pop ...\n"));
            if (s_empty(&ps->p_stack)) {
...
(const dfa).  A node in a const dfa represents a state of the parser; an arc represents
a transition.  Transitions are either labeled with terminal symbols or
with non-terminals.  When the parser decides to follow an arc labeled
with a non-terminal, it is invoked recursively with the const dfa representing
the parsing rule for that as its initial state; when that const dfa accepts,
the parser that invoked it continues.  The parse tree constructed by the
recursively called parser is inserted as a child in the current parse tree.

The const dfa's can be constructed automatically from a more conventional
...
(really: arc going out of a const dfa's state) must begin with a terminal
symbol.
...
The const dfa corresponding to the rule for expr is:
